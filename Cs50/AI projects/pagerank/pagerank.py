...
def main():
    ...
    for page in sorted(ranks):
        ...
        t += ranks[page]
    # TEST
    print("SURFER TOTAL: {:.4f}".format(t))
    ...
    for page in sorted(ranks):
        ...
        total += ranks[page]
    # TEST
    print("ITERATION TOTAL: {:.4f}".format(total))
...
def sample_pagerank(corpus, damping_factor, n):
    ...
    for i in range(n-1):
        ...
        # Choose a random key based on corresponding values and get its transition model
        pg = random.choices(options, weights = option_probs, k = 1)
        p = pg[0]
        t  = transition_model(corpus, p, damping_factor)
...
def iterate_pagerank(corpus, damping_factor):
    ...
    while diff > 0.001:
        # Use the prior iteration's ranks on this computation
        prev = copy.deepcopy(out)
        # Calculate the new probability for each page in corpus
        for p in corpus:
            v = first_part + calculate_sum(p, damping_factor, corpus, prev)
...
def num_links(p, corpus):
    ...
    o = len(corpus[p])
    # If the page has no links, it will be interpreted as linking to every page in the corpus
    if o == 0:
        return len(corpus)
    else:
        return o
...
def calculate_sum(page, damping_factor, corpus, page_ranks):
    ...
    p = corpus[page] # P is the set of pages this one links out to
    total = 0
    # A set of pages which link to this one
    links_to_this = set()

    # Find out which pages links to this one, if page has no out links, pretend it links to this
    for pg in list(corpus.keys()):
        if page in corpus[pg] or len(corpus[pg]) == 0:
            links_to_this.add(pg)

    # A page with no outbound links is interpreted as having a link to every page in the corpus + itself
    if len(links_to_this) == 0:
        print(page)
    if len(p) == 0:
